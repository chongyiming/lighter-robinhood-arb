...
_ZH = {
    ...
    "lighter-arb stopped": "lighter-arb 已停止",
...
class Dashboard:
        ...
        with Live(self._safe_render(), console=self.console,
            ...
            while not eng.stop.is_set():
                ...
                try:
                    ...
                    pass
        t = Text()
        t.append(self._t("lighter-arb stopped"), style="bold")
    ...
    def _render(self):
        ...
        if eng.base is None or eng.hedge is None or not eng.markets_ready:
            return Group(Panel(Text(self._t("starting — resolving markets…"),
                                    style="yellow"), title="lighter-arb",
    ...
    def _header(self):
        ...
        left = Text.assemble(("lighter-arb  ", "bold"),
                             (f"{cfg.symbol} × {eng.base.name} · {eng.hedge.name}",
                              "bold cyan"))
        right = Text()
    ...
    def _signal_panel(self):
        ...
        self._dir_row(t, self._t("SELL {b} → buy {h}", b=eng.base.name,
                                 h=eng.hedge.name),
                      eng.hedge, eng.base,
                      cfg.midline_bps + cfg.upper_bps, "sell_base")
        self._dir_row(t, self._t("BUY {b} → sell {h}", b=eng.base.name,
                                 h=eng.hedge.name),
                      eng.base, eng.hedge,
