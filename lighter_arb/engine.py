"""Two-venue arbitrage engine: a base leg against a hedge leg.
...
from .venue import LighterVenue
...
class Engine:
    ...
    async def _run_inner(self) -> None:
        ...
        if cfg.recorder_enabled or self.record_only:
            self.recorder = MinuteRecorder(cfg.recorder_csv, self.base.book,
    ...
    def _eff_threshold(self, buy, sell) -> float:
        ...
        selling base: executable premium must clear midline + upper;
        buying base: the reverse premium must clear lower - midline."""
        if sell.key == "base":
