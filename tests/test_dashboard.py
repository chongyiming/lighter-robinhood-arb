...
from lighter_arb.dashboard import BufferLogHandler, Dashboard  # noqa: E402
...
class StubVenue:
    ...
    def set_book(self, bid, ask):
        self.book.apply_lighter({"bids": [{"price": str(bid), "size": "10"}],
...
def test_renders_key_numbers():
    ...
    out = render(eng)
    for needle in ("LIGHTER", "RH", "SELL LIGHTER", "BUY LIGHTER",
...
def test_renders_in_chinese():
    ...
    for needle in ("实盘", "运行中", "交易所", "买一 / 卖一", "持仓", "会话",
                   ...
                   "卖出 LIGHTER → 买入 RH", "买入 LIGHTER → 卖出 RH",
                   "门槛 bps", "暂无执行", "日志事件", "秒前"):
