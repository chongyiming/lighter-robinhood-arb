...
from lighter_arb.book import OrderBook  # noqa: E402
from lighter_arb.config import load_config  # noqa: E402
from lighter_arb.engine import Engine  # noqa: E402
...
class StubVenue:
    ...
    def set_book(self, bid, ask, sz=50.0):
        self.book.apply_lighter({"bids": [{"price": str(bid), "size": str(sz)}],
                                 "asks": [{"price": str(ask), "size": str(sz)}]},
                                snapshot=True)
...
def make_engine(**thr):
    cfg = make_cfg(**thr)
    eng = Engine(cfg)
    eng.base = StubVenue("base", "LIGHTER")
...
def test_inventory_ladder():
    ...
    v = eng._inv_add_bps(e, h)                    # buying base adds long
    assert 7.5 < v < 8.5, v                       # u=0.9 -> ~+8
    approx(eng._inv_add_bps(h, e), 0.0)           # selling base reduces
...
def test_scan_respects_position_caps():
    ...
    eng.base.set_book(100.14, 100.16)
    eng.hedge.set_book(99.99, 100.01)
    eng.base.position = -100.0   # base leg already short at its cap
