#!/usr/bin/env python3
"""Entry point — arbitrage between two zkLighter deployments
(Lighter mainnet and Lighter on Robinhood Chain).
...
--symbol is required on every start: the market you trade is an explicit
decision, not a config default. The legs default to --base lighter and
--hedge lighter-rh; premium = base leg / hedge leg - 1. Add --cn for a
...
from lighter_arb.config import VENUES, ConfigError, load_config
from lighter_arb.engine import Engine
...
async def amain(cfg, record_only: bool, use_dashboard: bool, force_tty: bool,
    ...
    if not use_dashboard:
        await eng.run()
        return
    from lighter_arb.dashboard import Dashboard
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="LIVE arbitrage between Lighter mainnet and Lighter on "
    ...
    try:
        cfg = load_config(args.config, args.env_file, symbol=args.symbol,
                          base_venue=args.base, hedge_venue=args.hedge)
    ...
    if use_dashboard:
        try:
            from lighter_arb.dashboard import BufferLogHandler
        except ImportError:
